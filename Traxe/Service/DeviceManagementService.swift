        ...
        do {
            ...
            case 200:
                do {
                    ...
                    throw DeviceCheckError.decodingError(field: fieldName, swiftError: swiftDecodingError)
                } catch {
                    throw DeviceCheckError.decodingError(field: nil, swiftError: nil)
                }
            case 404:
                throw DeviceCheckError.notBitaxeDevice
            default:
...
enum DeviceCheckError: Error, LocalizedError {
    ...
    case decodingError(field: String?, swiftError: Swift.DecodingError?)
    ...
    var errorDescription: String? {
        ...
        case .decodingError(let field, _):
            if let fieldName = field, !fieldName.isEmpty {
                return "Could not understand the response from the device. Issue with data field: '\(fieldName)'."
            } else {
                return "Could not understand the response from the device (malformed data)."
            }
        case .notBitaxeDevice:
