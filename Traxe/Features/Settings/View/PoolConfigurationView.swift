    ...
    var body: some View {
        Form {
            Section("Pool Configuration") {
                TextField("Stratum URL (e.g., stratum.example.com)", text: $localStratumURL)
                    .keyboardType(.URL)
                    .autocorrectionDisabled(true)
                    .textInputAutocapitalization(.never)

                TextField("Stratum Port (e.g., 3333)", text: $localStratumPortString)
                    .keyboardType(.numberPad)

                TextField(
                    "Stratum User (e.g., YourBitcoinAddress.WorkerName)",
                    text: $localStratumUser
                )
                .autocorrectionDisabled(true)
                .textInputAutocapitalization(.never)
            }
